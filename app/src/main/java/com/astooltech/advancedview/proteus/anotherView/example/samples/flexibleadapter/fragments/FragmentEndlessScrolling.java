...
public class FragmentEndlessScrolling extends AbstractFragment
    ...
    private List<AbstractFlexibleItem> itemd;
    String nam;
    private ProteusView infl;
    private String layoutname;
public FragmentEndlessScrolling(ProteusView infli, String layoutnamee){
infl=infli;
this.nam=layoutnamee;
    getAllvalue eerr=new getAllvalue(infl,this.nam);

    this.itemd= eerr.getDataAll();
}
    ...
    public static FragmentEndlessScrolling newInstance(int columnCount,ProteusView infli, String layoutnamee) {
        FragmentEndlessScrolling fragment = new FragmentEndlessScrolling(infli,layoutnamee);
        Bundle args = new Bundle();
        args.putInt(ARG_COLUMN_COUNT, columnCount);
        fragment.setArguments(args);
        return fragment;
    ...
    private void initializeRecyclerView(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            mAdapter = new ExampleAdapter( this.itemd, getActivity());
        } else {
            mAdapter = new ExampleAdapter( this.itemd, getActivity());
        }
       ...
       FastScroller fastScroller = getView().findViewById(R.id.fast_scroller);
        fastScroller.addOnScrollStateChangeListener((ProtouseAbstractActivity) getActivity());
...
public void setfill(){
    mAdapter.hasNewFilter("12");
    mAdapter.setFilter("12");
}
    @Override
            ...
            public void run() {
               ...
               /* int count = new Random().nextInt(200);
                ...
                if (mAdapter.isTopEndless()) {
                    Collections.reverse(itemd);
                    DatabaseService.getInstance().addAll(0, itemd);
                } else {
                    DatabaseService.getInstance().addAll(itemd);
                }
                mAdapter.onLoadMoreComplete(itemd, (itemd.isEmpty() ? -1 : 3000L));
               ...
               // Log.d(TAG, "LastPosition=" + lastPosition);
                ...
                for (AbstractFlexibleItem item : itemd) {
                ...
                // 4. Notify user
                if (getActivity() != null && itemd.size() > 0) {
                    Toast.makeText(getActivity(),
                            "Simulated: " + itemd.size() + " new items arrived :-)",
    ...
    protected GridLayoutManager createNewGridLayoutManager() {
        ...
        gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            ...
            public int getSpanSize(int position) {
                ...
                int itemViewType = mAdapter.getItemViewType(position);//.recycler_scrollable_expandable_item:
