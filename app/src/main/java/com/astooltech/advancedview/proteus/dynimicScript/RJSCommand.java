package com.astooltech.advancedview.proteus.dynimicScript;
		...
		if (methodName.charAt(0) == RJSKeyword.OBJECT_ACCESS.toChar()) {
			...
			while (method.indexOf(RJSKeyword.DATA_POOL_COMMAND_OPEN_BRACE.toString(), lastCommandIndex) != -1) {
				...
				commandString = commandString.replace(RJSKeyword.METHOD_ARGUMENT_ACCESS.toString(),
						RJSKeyword.METHOD_ARGUMENT_ACCESS.toString() + methodId);
				int innerCommandIndex = new RJSResult(commandString, RJSResult.ResultType.COMMAND).getIndex();
		...
		if (!results.isEmpty() && results.get(0).getResult() != null)
			...
			if (indexOfLastDot != -1) {
				...
				try {
					...
					RJSParameter parameter = new RJSParameter(null, commandString, RJSParameter.ParameterType.FOR_ACCESS);
					List<RJSParameter> parameterList = new ArrayList<>();
					parameterList.add(parameter);
