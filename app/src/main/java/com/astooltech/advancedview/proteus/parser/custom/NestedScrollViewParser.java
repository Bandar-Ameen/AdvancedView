...
import com.astooltech.advancedview.proteus.view.PrototNestedScrollView;
...
public class NestedScrollViewParser<T extends NestedScrollView> extends ViewTypeParser<T> {
    ...
    public ProteusView createView(@NonNull ProteusContext context, @NonNull Layout layout, @NonNull ObjectValue data,
        ...
        return new PrototNestedScrollView(context);
    }
        ...
        addAttributeProcessor("Setting", new AttributeProcessor<T>() {
            ...
            public void handleValue(T view, Value value) {
                try {
                    ...
                    if (mode == 1) {
                        view.setOverScrollMode(NestedScrollView.OVER_SCROLL_NEVER);
                    }
                    if (mode == 2) {
                        view.setOverScrollMode(NestedScrollView.OVER_SCROLL_ALWAYS);
                    }
                    if (mode == 3) {
                        view.setOverScrollMode(NestedScrollView.OVER_SCROLL_IF_CONTENT_SCROLLS);
                    }
                }catch (Exception ex){
