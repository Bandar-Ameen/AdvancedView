...
private Layout ALayout;
    ...
    public ObjectValue getDataSource() {
        return DataSource;
    }

    public void setDataSource(ObjectValue dataSource) {
        DataSource = dataSource;
    }

    private ObjectValue DataSource;
 private boolean check;
    ...
    ProteusContext(Context base
                  ,
                   ...
                   String Stringstyles,Layout mroot,ObjectValue d,Gson cx, ProteusResources resources,StatuseLayout statuse,boolean check,Layout layot,ObjectValue datobj,ProteusLayoutInflater.ImageLoader im,ObjectValue daturce) {
        super(base);
        setStatusee(statuse);
        setCheck(check);
        setALayout(layot);
        setDataSource(daturce);
        setAData(datobj);
...
try {
    ...
    public  Value getDataFrom_DataLayout(String keyselect){
        try {
ObjectValue tempp= getAData();
            Value resul=null;
    ...
    public  Value getdata_Source(String keyselect){
        try {
            ObjectValue tempp= getDataSource();
            Value resul=null;
            String[] resull=keyselect.split("\\.");
            for(int z=0;z<resull.length;z++){
                try {
...
private StatuseLayout statu;
        ...
        public void setDataSourcee(ObjectValue dataSource) {
            DataSource = dataSource;
        }
        public Builder setDataSource(ObjectValue dataSource) {
            DataSource = dataSource;
            return  this;
        }
        private ObjectValue DataSource;
...
private Gson gsonn;
      ...
      //  private all_action_Activity setAllEventx;
        ...
        private String getstyleData(String jss){
         String jsj="{}";
          try {
              JSONObject js = new JSONObject(jss);

              Iterator<String> vv = js.keys();
              while (vv.hasNext()) {
                  try {
                      String c = vv.next();
                      if(c.toLowerCase().equals("style_layout")){
                        Gson h=new Gson();
                      String j=  h.toJson(js.get(c));
                          jsj=j;
...
setStyles(getstyleData(dataa));
            ...
            while (vv.hasNext()) {
                try {
                    ...
                    else if(ddertt.getKey().toLowerCase().equals("data_source")){
                        setDataSourcee(ddertt.getValue().getAsObject());
                    }
...
vvv.get("hh").getAsLayout().
          ...
          //  Log.e("ffff",layouts);
            ...
            return new ProteusContext(base, callback,containerrb,proteusManagerrb,dataURLl,massss,frgmentMangers,mFragmenttv,mDrawerr,ActvityAll,data,rootLayout,layouts,styles,RootLayout,Data,gsonn,resources,statu,false,null,new ObjectValue(),loader,getDataSource());
        }
...
vvv.get("hh").getAsLayout().
            ...
            return new ProteusContext(base, callback,containerrb,proteusManagerrb,dataURLl,massss,frgmentMangers,mFragmenttv,mDrawerr,ActvityAll,data,rootLayout,layouts,styles,RootLayout,Data,gsonn,resources,statu,true,layot,objj,loader,getDataSource());
        }
