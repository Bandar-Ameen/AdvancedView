            ...
            if (IDDdat.equals(datb.getIDUnit())) {
                if (typ.getAsString().equals("0")) {
                    ...
                    try {
                        ...
                        assert refresha != null;
                        refresha.removeFilter();
...
String sourceb=null;
                ...
                if(sourceb==null){
                   va=value;
                    ...
                    va = ((ProteusRecyclerView) view).getViewManager().getContext().getdata_Source(value.getAsObject().getAsString("source_name"));
                }
                    ObjectValue obj=va.getAsObject().getAsObject("Layout_manager");
    ...
    if (mode == 3) {
        view.setOverScrollMode(RecyclerView.OVER_SCROLL_IF_CONTENT_SCROLLS);
               ...
               Value ghh=  va.getAsObject().get("Data_From");
                boolean check=false;
try {
    Layout typlatgf = va.getAsObject().getAsLayout("Empty_View");
    Layout typlatgff = va.getAsObject().getAsLayout("Empty_View_Filter");
