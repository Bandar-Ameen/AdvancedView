...
import com.astooltech.advancedview.proteus.FunctionManager;
...
import com.astooltech.advancedview.proteus.toolbox.Result;
...
public abstract class Binding extends Value {
  ...
  public static final Pattern BINDING_PATTERNN = Pattern.compile("@@\\{fnn:(\\S+?)\\(((?:(?<!\\\\)'.*?(?<!\\\\)'|.?)+)\\)\\}|@\\{(.+)\\}");
  ...
  //    Log.i("ProteusEventWithTag", "vvvvvvvvvvvvvvvmmmmmmmmmmmmmmmmmmmmmkkk"+dd.toJson(data));
      if (tokens.length == 1 && INDEX.equals(tokens[0].value)) {
       // Log.i("ProteusEventWithTag", "vvvvvvvvvvvvvvvmmmmmmmmmmmmmmmmmmmmm");
        return Result.success(new Primitive(String.valueOf(index)));
      } else {
