...
private ProteusLayoutInflater minflater;
...
private String   getinflater(int pos){


   int viewType= holders.getViewType(items.get(pos).item, senderId);

    String inflatername=dataselect[0];
    if(viewType==131){
        inflatername=dataselect[1];
    }
    else if(viewType==130){
        inflatername=dataselect[2];
    }
    else if(viewType==-130){
        inflatername=dataselect[3];
    }
    else if(viewType==-131){
        inflatername=dataselect[4];
    }
    if(viewType==2){
        inflatername=dataselect[3];
    }
    ...
    //   Log.i("00","hhh"+viewType);
       ...
       if(viewType==131){
        ...
        return holders.getHolder(parent, viewType, messagesListStyle,inflatername,minflater);
    }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        if (wrapper.item instanceof IMessage) {
           // DataContext context = DataContext.create(holder.itemViews.getViewManager().getContext(), ((IMessage) wrapper.item).getAlldata(), position, holder.itemViews.getViewManager().getDataContext().getScope());

/*if(wrapper.refresh==0) {*/
    wrapper.refresh=1;
    Gson ty = new Gson();
    //  Log.i("888",  ty.toJson(holder.itemViews.getViewManager().getDataContext()));
    //.setVisibility(View.GONE);
    ProteusView cx = holder.itemViews.getViewManager().getContext().getInflater().inflate(getinflater(position), ((IMessage) wrapper.item).getAlldata());

    // holder.itemViews.getAsView().getRootView().=cx;
    ((ViewGroup) holder.itemView.getRootView()).removeAllViews();
    ((ViewGroup) holder.itemView.getRootView()).addView(cx.getAsView());
    //=cx.getAsView();
    ((ProteusView) holder.itemView).getViewManager().getDataContext().update(holder.itemViews.getViewManager().getContext(), ((IMessage) wrapper.item).getAlldata());
//}
        }

     //  holder.itemViews.getViewManager().update();
     ...
     */
    public void addToStart(MESSAGE message, boolean scroll,ProteusLayoutInflater minflaterr,String[] u) {

        this.minflater=minflaterr;
        this.dataselect=u;
    ...
    public void upsert(MESSAGE message) {
        if (!update(message)) {
            addToStart(message, false,minflater,dataselect);
        }
    }
        ...
        return new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (selectionListener != null && isSelectionModeEnabled) {
                    ...
                    if (wrapper.isSelected) MessagesListAdapter.this.incrementSelectedItemsCount();
                    else MessagesListAdapter.this.decrementSelectedItemsCount();
...
public  int refresh=0;
        Wrapper(DATA item,int refreshh) {
            this.item = item;
            this.refresh=refreshh;
        }
        Wrapper(DATA item) {
            this.item = item;
...
*/
    ...
    public static class IncomingMessageViewHolder<MESSAGE extends IMessage>
        ...
        public IncomingMessageViewHolder(ProteusView itemView) {
            super(itemView);
        }
    }
    ...
    public static class OutcomingMessageViewHolder<MESSAGE extends IMessage>
        ...
        public OutcomingMessageViewHolder(ProteusView itemView) {
            super(itemView);
        }
    }
}
