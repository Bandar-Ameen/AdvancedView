...
public class protusePasswordEditTexParser<T extends PasswordEditText> extends ViewTypeParser<T> {
    @NonNull
    @Override
    ...
    public ProteusView createView(@NonNull ProteusContext context, @NonNull Layout layout, @NonNull ObjectValue data,
        ...
        return new protusePasswordEditText(context);
    }
    ...
    public void GetAndSetData(ProteusView view, DataValueSelect data, int typoper, Object anotherdat, String viewname) {
        ...
        if(view instanceof protusePasswordEditText){
            SetGetFindTextInputLayoutEditTextCheckBoxSwiRedmor((protusePasswordEditText )view,data,typoper);
        }
    }
    private  void SetGetFindTextInputLayoutEditTextCheckBoxSwiRedmor( protusePasswordEditText  dt,DataValueSelect datb,int typoper) {
        if (typoper == 1) {
    ...
    private void getwithanotheroper(protusePasswordEditText  dt, DataValueSelect datb, int typoper){
        protusePasswordEditText  ccc=dt;
        if (typoper == 2) {
    ...
    protected void addAttributeProcessors() {
        addAttributeProcessor("child", new AttributeProcessor<T>() {
            ...
            public void handleValue(T view, Value value) {
                Layout dir=value.getAsObject().getAsLayout("icona");
                Layout dirr=value.getAsObject().getAsLayout("iconb");
                protusePasswordEditText rr=(protusePasswordEditText)view;
                             ProteusView e=  rr.getViewManager().getContext().getInflater().inflate(dir,new ObjectValue());
                ProteusView ee=  rr.getViewManager().getContext().getInflater().inflate(dirr,new ObjectValue());

               // protouseInboxBackgroundScrollView er=(protouseInboxBackgroundScrollView)e.getAsView();


                rr.setmHidePwDrawable(e.getAsView().getBackground());
                rr.setmShowPwDrawable(ee.getAsView().getBackground());
