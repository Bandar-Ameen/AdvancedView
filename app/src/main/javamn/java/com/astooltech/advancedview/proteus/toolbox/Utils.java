...
public class Utils {
  ...
  public static ObjectValue addAllEntries(@NonNull ObjectValue destination, @NonNull ObjectValue source) {
    for (Map.Entry<String, Value> entry : source.entrySet()) {
     try {
       for (Map.Entry<String, Value> entryy : entry.getValue().getAsObject().entrySet()) {
         if (destination.get(entryy.getKey()) != null) {
           continue;
         }
         destination.add(entryy.getKey(), entryy.getValue());
       }
     }catch (Exception ex){
