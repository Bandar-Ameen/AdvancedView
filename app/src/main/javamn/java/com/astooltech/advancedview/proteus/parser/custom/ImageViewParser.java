...
public class ImageViewParser<T extends ImageView> extends ViewTypeParser<T> {
  ...
  public void GetAndSetData(ProteusView view, DataValueSelect data, int typoper, Object anotherdat, String viewname) {
    ...
    if(view instanceof ProteusImageView){
      SetImage((ProteusImageView )view,data,typoper);
    }
  }
  ...
  private  void getwithanotheroper(ProteusImageView dt, DataValueSelect datb, int typoper){
    ProteusImageView   ccc = dt;
    if (typoper == 2) {
  ...
  private void SetImage(ProteusImageView  dt,DataValueSelect datb,int typoper) {
    if (typoper == 1) {
