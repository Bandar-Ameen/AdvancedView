...
import com.astooltech.advancedview.proteus.view.custom.badge.ProteusBage;

import java.util.Iterator;
import java.util.List;
import java.util.Map;

public class ProtouseSkeletonViewGroupParser<T extends SkeletonViewGroup> extends ViewTypeParser<T> {
    ...
    private static ProtouseSkeletonViewGroup getview;
    @Override
...
try {
    if (getview != null) {
        ...
        getview.getViewManager().getActionEventView().sendEvent(null, 155, Broad);
    }
}catch (Exception ex){
    ...
    public ProteusView createView(@NonNull ProteusContext context, @NonNull Layout layout, @NonNull ObjectValue data,
        ...
        getview=new ProtouseSkeletonViewGroup(context);
        return getview;
    ...
    public void GetAndSetData(ProteusView view, DataValueSelect data, int typoper, Object anotherdat, String viewname) {
        ...
        if(view instanceof ProtouseSkeletonViewGroup){
            SetGetFindTextInputLayoutEditTextB((ProtouseSkeletonViewGroup)view,data,typoper);
        }
    }

    private boolean checknullvalues(ProtouseSkeletonViewGroup dt){
        boolean checkvalue=false;
        ProtouseSkeletonViewGroup ccc = dt;
        try {
    ...
    private void getwithanotheroper(ProtouseSkeletonViewGroup  dt, DataValueSelect datb, int typoper){
        ProtouseSkeletonViewGroup ccc = dt;
        if (typoper == 2) {
    ...
    private  void SetGetFindTextInputLayoutEditTextB(ProtouseSkeletonViewGroup dt, DataValueSelect datb, int typoper) {
        if (typoper == 1) {
    ...
    protected void addAttributeProcessors() {
        addAttributeProcessor("Daaa", new AttributeProcessor<T>() {
                    @Override
                    public void handleValue(final T view, Value value) {
                        ...
                        int startfrom=value.getAsObject().getAsInteger("Start_From");
...
view.setSkeletonListener(new SkeletonViewGroup.SkeletonListener() {
    @Override
    ...
    public void onFinishAnimation() {
       // textTv.setText("The Android O release ultimately became Android 8.0 Oreo, as predicted by pretty much everyone the first time they thought of a sweet");
                        ...
                        ProtouseSkeletonViewGroup ttui=   (ProtouseSkeletonViewGroup)view;
