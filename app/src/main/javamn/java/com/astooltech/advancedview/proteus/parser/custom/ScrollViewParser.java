...
import com.astooltech.advancedview.proteus.view.ProteusScrollView;
    ...
    addAttributeProcessor("Setting", new AttributeProcessor<T>() {
      ...
      public void handleValue(T view, Value value) {
        try {
          ...
          if (mode == 2) {
            view.setOverScrollMode(ScrollView.OVER_SCROLL_ALWAYS);
          }
          if (mode == 3) {
            view.setOverScrollMode(ScrollView.OVER_SCROLL_IF_CONTENT_SCROLLS);
          }
        }catch (Exception ex){
