...
import com.astooltech.advancedview.proteus.anotherView.example.samples.flexibleadapter.FlexHolderItem;
...
import com.astooltech.advancedview.proteus.anotherView.flexibleadapter.common.TopSnappedSmoothScroller;
...
public class FragmentStaggeredLayout extends AbstractFragment {
    ...
    private List<AbstractFlexibleItem> dtn;
   private List<HiderWithbody> dfff;
    private  boolean chek;
    private FlexibleAdapter<AbstractFlexibleItem> mAdapter;
   private Value datasetengg;
   private  int shoseval=0;
    private ProteusLayoutInflater inflatg;
    ...
    public static FragmentStaggeredLayout newInstance(int columnCount,List<AbstractFlexibleItem> cv) {
        FragmentStaggeredLayout fragment = new FragmentStaggeredLayout(cv);
        Bundle args = new Bundle();
        args.putInt(ARG_COLUMN_COUNT, columnCount);
        fragment.setArguments(args);
        return fragment;
    }
    public static FragmentStaggeredLayout newInstance(int columnCount,List<HiderWithbody> dtnnn, int c) {
        FragmentStaggeredLayout fragment = new FragmentStaggeredLayout(dtnnn,c);
        Bundle args = new Bundle();
        args.putInt(ARG_COLUMN_COUNT, columnCount);
        fragment.setArguments(args);
        return fragment;
    }
    public static FragmentStaggeredLayout newInstance(int columnCount,Value dataseteng,  ProteusLayoutInflater inflat, int c) {
        FragmentStaggeredLayout fragment = new FragmentStaggeredLayout(dataseteng,inflat,c);
        Bundle args = new Bundle();
        args.putInt(ARG_COLUMN_COUNT, columnCount);
        fragment.setArguments(args);
        return fragment;
    ...
    public FragmentStaggeredLayout(List<AbstractFlexibleItem> dtnn) {
        chek=true;
        dtn=dtnn;
        dfff=null;
    }
    public FragmentStaggeredLayout(List<HiderWithbody> dtnn,int a) {
        chek=true;
        dfff=dtnn;
    }

    public FragmentStaggeredLayout(Value dataseteng,  ProteusLayoutInflater inflat,int a) {
        chek=true;
        shoseval=1;
        datasetengg=dataseteng;
        inflatg=inflat;
    ...
    private void initializeRecyclerView(Bundle savedInstanceState) {
        ...
        List<ScrollableUseCaseItemCustome> hiderscrol=null;
      if(chek) {
if( shoseval==1){
    FlexHolderItem dr = new FlexHolderItem();
    List<AbstractFlexibleItem> r = dr.getDataAllDataFromStringg(datasetengg,inflatg);

    hiderscrol=dr.getHeadersrcoll();
    mAdapter = new FlexibleAdapter<>(r, getActivity());
}else {
    if (dfff != null) {
        FlexHolderItem dr = new FlexHolderItem();
        List<AbstractFlexibleItem> r = dr.createStaggeredDatabase(dfff);
        mAdapter = new FlexibleAdapter<>(r, getActivity());

    } else {
        mAdapter = new FlexibleAdapter<>(dtn, getActivity());
    }
}
...
if(hiderscrol.size()>0){
    mAdapter.addScrollableHeader(hiderscrol.get(0));
}
                ...
                if (dfff != null) {
                    ...
                    mAdapter.addScrollableHeader(new ScrollableUseCaseItem(
                            dfff.get(0).getScrollHeder().getScrolitemtit(),
                            dfff.get(0).getScrollHeder().getSrolitem()));
