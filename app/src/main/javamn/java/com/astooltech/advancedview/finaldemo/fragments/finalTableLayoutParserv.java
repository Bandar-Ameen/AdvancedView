 ...
 */
public class finalTableLayoutParserv<T extends LinearLayout> extends ViewTypeParser<T> {
            ...
            private void cxc(ViewGroup dfdf,View v,View header,String TitleViewID,String Continer){
                for (int xxc = 0; xxc < dfdf.getChildCount(); xxc++) {
                    ...
                    if(dfdf.getChildAt(xxc) instanceof  ViewGroup){
                        ...
                        boolean check=false;
                        try{
                            if(j.getTag(com.astooltech.advancedview.R.id.tag3)!=null) {
                                if (j.getTag(com.astooltech.advancedview.R.id.tag3).toString().equals(TitleViewID)) {
                                    check = true;

                                    j.addView(header);

                                }else     if (j.getTag(com.astooltech.advancedview.R.id.tag3).toString().equals(Continer)) {
                                    check = true;
                    ...
                    ProteusView.Manager.ActionEventView Act = new ProteusView.Manager.ActionEventView() {
                        ...
                        public List<AbstractFlexibleItem> createOverallDatabase(Resources resources, List<ProteusView> itt) {
                            ...
                            for (int cx = 0; cx < itt.size(); cx++) {
                                ...
                                mItems.add(new OverallItem(com.astooltech.advancedview.R.id.nav_selection_modes, resources.getString(com.astooltech.advancedview.R.string.selection_modes)).withView(itt.get(cx).getAsView()).withcheck(false)
                                        .withDescription(resources.getString(com.astooltech.advancedview.R.string.selection_modes_description))
                                        .withIcon(resources.getDrawable(com.astooltech.advancedview.R.drawable.ic_select_all_grey600_24dp)));
                            }
