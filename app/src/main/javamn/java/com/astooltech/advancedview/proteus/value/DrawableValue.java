...
import com.astooltech.advancedview.proteus.toolbox.SimpleArrayIterator;
...
public abstract class DrawableValue extends Value {
  ...
  private static final String CustomeFontdrable = "fonticon";
  ...
  public static DrawableValue valueOf(ObjectValue value, Context context) {
    ...
    switch (type) {
      ...
      case CustomeFontdrable:
        return FontMydrable.valueOf(value, context);
        default:
        return null;
    }
  }
  ...
  public static class ShapeValue extends DrawableValue {
    ...
    private static final int shapdraw = 99;
    private static final int shapcustom = 199;
    private static final String SHAPE_RECTANGLE = "rectangle";
    private static final String SHAPE_OVAL = "oval";
    private static final String Shap_Font = "shape_font";
    private static final String Shap_Custome = "Shape_Custome";
    private static final String SHAPE_LINE = "line";
    private static final String SHAPE_RING = "ring";
    private static  final String mytap="draw";
    private  static   String typp="typm";
    ...
    private ShapeValue(ObjectValue value, Context context) {
      ...
      customDrawb=null;
      if(this.shape==99) {
  typp = value.getAsObject(mytap).get("icon_name").toString();
  icon_size = value.getAsObject(mytap).get("icon_size").getAsInt();
  icon_color = value.getAsObject(mytap).get("icon_color").toString();
}
      if(this.shape==199) {
        try {
          customDrawb = GetDrawbileFromString.getDrawble(value.getAsObject("Shape_Path"), context);
        } catch (Exception ex) {

        }
      }
      // Log.i("999",h.toJson(value.getAsObject(mytap)));
...
String dde=typp; //"\uf196";
         char SPACE_SHUTTLE_CODE =  dde.charAt(0); //'\uf198';



        /*AFontDrawable drawablex = new AFontDrawable(view.getAsView().getContext(), R.string.app_name, false, true);
        drawablex.setTextColor(ParseHelper.parseColor(icon_color));
        drawablex.setTextSize(icon_size);*/
        @ColorInt
         int MATERIAL_BLUE = ParseHelper.parseColor(icon_color); // .getAsInt();
        FontDrawable spaceShuttle = new FontDrawable.Builder(view.getAsView().getContext(), SPACE_SHUTTLE_CODE, CUSTOM_FONT_PATH)
                .setSizeDp(icon_size)
                .setColor(MATERIAL_BLUE)
                .build();
        callback.apply(spaceShuttle);
  ...
  public static class FontMydrable extends DrawableValue {
   ...
   // private static final String DRAWABLE_STR = "drawable";
    ...
    private  static   int icon_size=50;
    private  static   int typfon=1;
    private  static   String icon_color;

    private FontMydrable(ObjectValue value, Context context) {

        typp = value.getAsObject(mytap).get("icon_name").toString();
        icon_size = value.getAsObject(mytap).get("icon_size").getAsInt();
        icon_color = value.getAsObject(mytap).get("icon_color").toString();
      typfon = value.getAsObject(mytap).get("icon_type").getAsInt();

    }
    public static FontMydrable valueOf(ObjectValue layers, Context context) {
      return new FontMydrable(layers, context);
    }
   /* private final int[] ids;
    private final Value[] layers;

    public FontMydrable(int[] ids, Value[] layers) {
      this.ids = ids;
      this.layers = layers;
    }
*/
    ...
    public void apply(ProteusView view, Context context, ProteusLayoutInflater.ImageLoader loader, Callback callback) {
     ...
     try {
       ...
       @ColorInt
       int MATERIAL_BLUE = ParseHelper.parseColor(icon_color);
...
//Log.i("7777",url);
  ...
  public abstract static class AsyncCallbackicon {

    private boolean recycled;

    protected AsyncCallbackicon() {
    }
